...
def new_win_calculation(board, row, col, piece):
    if col == 3:
        min_col = 0
        max_col = 6
    elif col < 3:
...
def score_position(board, piece):
    ...
    for r in range(ROW_COUNT - 3):
        for c in range(COLUMN_COUNT - 3):
            window = [board[r+1][c+1] for i in range(WINDOW_LENGTH)]
            score += evaluate_window(window, piece)
...
while not game_over:
    ...
    if turn == AI and not game_over:
        ...
        col, minimax_score = minimax(board, 6, -float('inf'), float('inf'), True)
